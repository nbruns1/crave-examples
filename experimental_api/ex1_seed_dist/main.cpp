...
class item : public crv_sequence_item {
 public:
  item(crv_object_name) {
    c_src_addr_range = { dist(src_addr(), make_distribution(range<unsigned int>(0, 9), range<unsigned int>(90, 99))) };
    c_dest_addr_range = { dist(dest_addr(),
                               make_distribution(weighted_range<unsigned int>(0, 9, 60), weighted_range<unsigned int>(10, 19, 30),
                                                 weighted_range<unsigned int>(100, 109, 10))) };
  }
